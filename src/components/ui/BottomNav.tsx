...
export default function BottomNav() {
  ...
  const { effectiveRole, actualRole } = useAuth();
  ...
  const navItems = [
    ...
    { path: notesUrl, label: t("orders.notes"), icon: StickyNote, external: true },
  ];
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-dark-800 border-t border-dark-600 safe-bottom z-40">
      <div className="flex justify-around items-center h-16">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = !("external" in item) && (pathname.startsWith(item.path) || (item.path === "/orders" && pathname === "/"));
